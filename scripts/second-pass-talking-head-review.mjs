...
function renderReviewPacket({ args, pass1Video, pass1TranscriptPath, silencePath, silenceEvents, gaps, probePath }) {
  const lines = [
    ...
    "- Original prep: `takes_packed.md`, `pre_scan.md`, `silence_scan/*.json`, `transcripts/*.json`",
    "",
    "## Agent Task",
    "",
    ...
    "5. Reopen the original material packet before deciding the final EDL. Use `takes_packed.md`, `pre_scan.md`, and `transcripts/*.json` to replace weak first-pass ranges, recover better alternate takes, or add omitted useful detail when time allows.",
    "6. Write `edl_final.json` and `editor_qc.md`.",
    "7. Run `npm run talking-head:review-script -- --edit-dir <edit_dir>` and read `review_script.md` before accepting the review render.",
    "8. In `editor_qc.md`, explicitly mention `Second pass review`, `Original source reconsideration`, and whether `review_script.md` exposed repetition or wording issues.",
    "",
    "## Required Second-Pass Outputs",
    "",
    "- `edl_final.json`: the best review EDL after the second pass.",
    "- `editor_qc.md`: notes on changes, source reconsideration, repetition checks, and remaining risks.",
    "- `review_script.md`: readable final script with output timestamps for user and agent review.",
    "",
